...
public class Scheme18 extends BaseSchemeUniformOnly {

    //full
    static List<Formula> formulas = new ArrayList<Formula>() {{
        add(new Formula(932, "Ф1(+0,25) – 12 – Ф2(+0,25)", BetType.F1plus025, BetType.P12, BetType.F2plus025));
    }};
    ...
    public List<EventSum> fillProfit(List<EventSum> bets, Formula f, Double C1, Double C2, Double C3,
        ...
        for (EventSum sum: bets) {
            if (sum.bet == f.var1) {
                ...
                sum.sum = C2;
                sum.profit = C1*(K1 + 1)/2 + C3*(K3 + 1)/2;
    ...
    protected BranchResult calculateUniform(List<EventSum> bets, Formula f, Double totalSum) {
        ...
        Double X1 = (2 * K1 + (K1 + K3) / K2 + 2 * K3) / (2 * K1 * K3 + K1 + K3);
        Double X2 = (K1 + K3) / (2 * K2 * K3);
        Double X3 = K1 / K3;
